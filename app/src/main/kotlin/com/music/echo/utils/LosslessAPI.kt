...
object LosslessAPI {
    ...
    private suspend fun fetchMusicList(forceRefresh: Boolean = false): List<LosslessTrack> = withContext(Dispatchers.IO) {
        val now = System.currentTimeMillis()
        if (!forceRefresh && cachedIndex != null && (now - lastFetchTime) < 1000 * 60 * 60) {
    ...
    suspend fun search(queryTitle: String, queryArtist: String, forceRefresh: Boolean = false): LosslessTrack? {
        val list = fetchMusicList(forceRefresh)
