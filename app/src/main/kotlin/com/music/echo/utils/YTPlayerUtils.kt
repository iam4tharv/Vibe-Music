...
import com.music.echo.utils.BotDetectionMitigator
...
import com.music.echo.utils.YTPlayerUtils.MAIN_CLIENT
import com.music.echo.utils.YTPlayerUtils.STREAM_FALLBACK_CLIENTS
import com.music.echo.utils.YTPlayerUtils.validateStatus
import com.music.echo.utils.potoken.PoTokenGenerator
import com.music.echo.utils.potoken.PoTokenResult
import com.music.echo.utils.sabr.EjsNTransformSolver
import com.music.echo.utils.PlaybackLogLevel
...
object YTPlayerUtils {
    ...
    suspend fun playerResponseForPlayback(
        ...
        val showFallbackToast = context?.let { 
            it.dataStore.data.first()[com.music.echo.constants.ShowAudioFallbackToastKey] 
        } ?: false

        var hasShownLosslessToast = false
        var hasShownJioSaavnToast = false
        var hasShownOpusToast = false

        suspend fun tryJioSaavn(): Result<PlaybackData> {
            var attemptResult: Result<PlaybackData>? = null
            var lastException: Exception? = null
            try {
                attemptResult = kotlinx.coroutines.withTimeoutOrNull(10000L) {
                    ...
                    if (title != null && author != null) {
                        ...
                        if (track != null) {
                            ...
                            throw Exception("No streamable match resolved on JioSaavn")
                        }
                    } else {
                ...
                if (attemptResult == null) {
                    lastException = Exception("Timeout fetching JioSaavn stream")
                }
            } catch (e: Exception) {
                lastException = e
            }
        ...
        suspend fun tryLossless(): Result<PlaybackData> {
            ...
            try {
                attemptResult = kotlinx.coroutines.withTimeoutOrNull(10000L) {
                    ...
                    val author = knownArtist ?: metadata?.videoDetails?.author?.replace(" - Topic", "")
                    if (title != null && author != null) {
                        val track = com.music.echo.utils.LosslessAPI.search(title, author, false)
                        if (track != null) {
                            val format = com.music.innertube.models.response.PlayerResponse.StreamingData.Format(
                                itag = 0,
        ...
        return when (audioQuality) {
            AudioQuality.JIOSAAVN -> {
                val saavnRes = tryJioSaavn()
                if (saavnRes.isSuccess) return saavnRes

                Timber.tag(TAG).e("JioSaavn resolution failed, falling back to YouTube Opus")
                if (!hasShownJioSaavnToast) {
                    hasShownJioSaavnToast = true
                    showToastMsg(if (isDownload) "JioSaavn download unavailable, falling back to Opus" else "JioSaavn stream unavailable, falling back to Opus")
                }
    ...
    private suspend fun resolvePlaybackData(
        ...
        if (isAgeRestrictedFromResponse) {
            
            Timber.tag(logTag).d("Age-restricted detected, using TVHTML5_SIMPLY_EMBEDDED_PLAYER")
            Log.i(TAG, "Age-restricted: using TVHTML5_SIMPLY_EMBEDDED_PLAYER for videoId=$videoId")
            val creatorResponse = YouTube.player(videoId, playlistId, TVHTML5_SIMPLY_EMBEDDED_PLAYER, null, null)
                .onFailure {
                    ...
                    Timber.tag(logTag).e(it, "player() request FAILED for TVHTML5_SIMPLY_EMBEDDED_PLAYER")
                }.getOrNull()
            if (creatorResponse?.playabilityStatus?.status == "OK") {
                Timber.tag(logTag).d("TVHTML5_SIMPLY_EMBEDDED_PLAYER works for age-restricted content")
                mainPlayerResponse = creatorResponse
                usedAgeRestrictedClient = TVHTML5_SIMPLY_EMBEDDED_PLAYER
            }
        }

        // Do not throw here if mainPlayerResponse is null

        
        
        var audioConfig = metadataResponse?.playerConfig?.audioConfig ?: mainPlayerResponse?.playerConfig?.audioConfig
        var videoDetails = metadataResponse?.videoDetails ?: mainPlayerResponse?.videoDetails
