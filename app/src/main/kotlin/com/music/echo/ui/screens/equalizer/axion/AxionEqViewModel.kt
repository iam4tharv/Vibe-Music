package com.music.echo.ui.screens.equalizer.axion
...
class AxionEqViewModel @Inject constructor(
    ...
    private val _fxBoosterEnabled = MutableStateFlow(prefs.getBoolean("fx_booster", true))
    val fxBoosterEnabled = _fxBoosterEnabled.asStateFlow()
    ...
    fun setFxBoosterEnabled(enabled: Boolean) {
        _fxBoosterEnabled.value = enabled
        prefs.edit().putBoolean("fx_booster", enabled).apply()
        if (_enabled.value) {
            applyToService()
        }
    }
    ...
    private fun applyToService() {
        viewModelScope.launch {
            ...
            val profile = SavedEQProfile(
                ...
                deviceModel = "Equalizer",
                bands = bands,
                preamp = if (_fxBoosterEnabled.value) 5.0 else 0.0,
                isCustom = false,
                isActive = true
            )
